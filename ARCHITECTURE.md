...
[`QaWebli.Infrastructure`](src/QaWebli.Infrastructure/) handles parsing quiz files from disk and writing audit logs. [`QaWebli.TerminalUI`](src/QaWebli.TerminalUI/) exercises the domain in a Spectre.Console terminal demo with the Strategy pattern for rendering; higher-level concerns (web UI) will live in other layers as they are implemented.
...
* **How it works step-by-step:**
  ...
  2. `PlainTextRendererStrategy` always returns `true` for `CanRender` and renders the question's `RawText` as escaped white Spectre markup inside a cyan-bordered panel.
